...
const initScrollReveal = () => {
  ...
  if (firma) {
    const firmaObs = new IntersectionObserver(entries => {
      entries.forEach(e => {
        if (e.isIntersecting) {
...
const initFinalBurst = () => {
  ...
  const fireParticles = () => {
    ...
    const colors = ['#e8306a', '#ff6b9d', '#ffb3d0', '#fff', '#ffd6e7', '#c01555'];
    const shapes = ['💘', '✦', '✧', '♡', '✿'];
    ...
    for (let i = 0; i < 60; i++) {
      ...
      const dist = rand(100, Math.min(window.innerWidth, window.innerHeight) * 0.6);
    ...
    if (finalHeart) {
      ...
      finalHeart.style.transition = 'transform 0.5s cubic-bezier(0.34,1.56,0.64,1)';
      setTimeout(() => {
